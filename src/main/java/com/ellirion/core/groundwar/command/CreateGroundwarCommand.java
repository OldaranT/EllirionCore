...
public class CreateGroundwarCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (race == null) {
            player.sendMessage(ChatColor.DARK_RED + "You are not in a race, therefore you are not eligible to start a ground war.");
            return true;
        }

        GroundWarManager.addGroundWar(playerId);
        player.sendMessage(ChatColor.GREEN + "Successfully started a ground war. Now select plots by using the /wagerplot command");
